...
//     long *shm_taskid;
//
//     shmid1 = shmget(getpid() + SHMKEY1, nr_cpus, S_IRWXU | S_IRWXG | S_IRWXO | IPC_CREAT);
//   	if (shmid1 == -1) { perror("shmget"); exit(1); }
//   	shm_taskid = (long*) shmat(shmid1, (void*)0, 0);
//   	memset(shm_taskid, -1, nr_cpus);
//
...
//   	memset(shm_data_out, 0, data_out_size);
//
// }
//
// void mp_task_mgr2(int nr_cpus, int nr_pnts, void *data_in, void *data_out, int data_out_size, void (*parallelized_function)(int cpu_id, int data_id, void *data_in, void *shm_data_out)) {
//
