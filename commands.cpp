...
int cmd_dynap_fmap(const std::vector<std::string>& args) {
	...
	Pos<double> p0(0,0,0,0,de,0);
	std::vector<DynApGridPoint> grid;
	dynap_fmap(accelerator, cod, nr_turns, p0, x_nrpts, x_min, x_max, y_nrpts, y_min, y_max, true, grid);
	...
	status = print_dynapgrid (accelerator, grid, "[dynap_fmap]", "dynap_fmap_out.txt");
	if (status == Status::file_not_opened) return status;
